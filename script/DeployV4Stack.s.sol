...
import { MockWETH9 } from "../src/test-only/MockWETH9.sol";
...
contract DeployV4Stack is Script {
    ...
    function run() external {
        ...
        address existingUsdc = vm.envOr("MOCK_USDC", address(0));
        address existingNvda = vm.envOr("MOCK_NVDA", address(0));
        ...
        MockToken mockUsdc =
            existingUsdc == address(0) ? new MockToken("Mock USD Coin", "mUSDC", 6) : MockToken(existingUsdc);
        MockToken mockNvda =
            existingNvda == address(0) ? new MockToken("Mock NVIDIA", "mNVDA", 18) : MockToken(existingNvda);

        if (existingUsdc == address(0)) mockUsdc.mint(deployer, 1_000_000e6);
        if (existingNvda == address(0)) mockNvda.mint(deployer, 10_000e18);

        mockUsdc.approve(address(router), type(uint256).max);
        mockNvda.approve(address(router), type(uint256).max);
        ...
        (PoolKey memory key, bool usdcIsToken0) = _poolKey(address(mockUsdc), address(mockNvda), address(0));
        ...
        console2.log("MockWETH9:", address(weth9));
        ...
        console2.log("MockNVDA:", address(mockNvda));
