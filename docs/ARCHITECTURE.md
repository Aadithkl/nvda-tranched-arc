...
**Interactive maps** (self-contained HTML, open in a browser): [system architecture](archify/nvda-tranches.architecture.html) and [deposit to JIT swap](archify/nvda-deposit-jit.sequence.html). Typed sources live in `docs/archify/`; regenerate with the Archify skill (validate, then deliver).

Contracts and scripts are linked with line numbers so reviewers can verify each claim in the
source. Addresses are never embedded here: read them from `deployments/arc-testnet.json`
...
Interactive maps: typed sources in `docs/archify/`, self-contained HTML next to them; regenerate with the Archify skill (validate, then deliver). The legacy Mermaid sources remain in `docs/diagrams/` with exports in `docs/assets/` (`npm run diagrams:export`).