...
# the in-repo app is vanilla TS + Vite (frontend/); no framework required
```
...
## Tranche stack (dual-token USDC/equity)
...
Live deployment is the v3 USDC/NVDA stack (JIT quoting active; vaults funded):

| Contract | Address |
|---|---|
| `TrancheJITHook` | `0x5C374e0B4F3646705839BE9D2b45F6753EAC6aC0` |
| `TranchePipeModule` | `0x04614f09DfC7D66B5072FB9B745C9B1b9503bA5e` |
| `HookShareToken` | `0x9341fA835A44A225E7f36a245A149794239c221A` |
...
exits remain available when the oracle is stale. Prices are published by an owner-managed writer —
there is no public push path.
...
closed. The agent (the authorized oracle writer) refreshes it automatically on load and
each tick; every other address reverts `NotWriter`. The UI should show "stale / market
closed" from `valid`, `marketStatus`, and `sourceTimestamp`.
