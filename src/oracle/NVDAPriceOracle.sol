...
contract NVDAPriceOracle {
    ...
    function verifyAndUpdate(bytes calldata unverifiedReport) external returns (DataStreamsV11.Report memory report) {
        ...
        if (report.marketStatus == DataStreamsV11.STATUS_CLOSED || report.marketStatus == DataStreamsV11.STATUS_UNKNOWN)
        {
            return report;
        }
        ...
        returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound)
    {
        ...
        StoredPrice memory stored = _prices[session];
        if (stored.observationsTimestamp == 0) revert NoPriceData(session);
        return (
            uint80(stored.observationsTimestamp),
            int256(stored.mid),
            uint256(stored.observationsTimestamp),
            stored.updatedAt,
            uint80(stored.observationsTimestamp)
        );
    }
    ...
    function description() external pure returns (string memory) {
        return "NVDA/USD (Chainlink Data Streams)";
    }
